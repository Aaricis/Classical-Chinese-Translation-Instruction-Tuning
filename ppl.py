...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="(Optional) Path to LoRA adapter. If not provided, use base model only.",
    )
    parser.add_argument(
        "--test_data_path",
        type=str,
    ...
    if args.peft_path:
        # Load LoRA
        print(f"Loading LoRA adapter from: {args.peft_path}")
        model = PeftModel.from_pretrained(model, args.peft_path)
    else:
        print("Using base pretrained model only (no LoRA applied)")
